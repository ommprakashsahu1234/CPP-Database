...
vector<vector<string>> loadQueries(const string &filename) {
...
void viewQueriesAndRespond() {
    const string filename = "queries.csv";
    vector<vector<string>> queries = loadQueries(filename);
    
    // Check for unanswered queries
    if (queries.empty()) {
        cout << "No unanswered queries found." << endl;
        return;
    }

    cout << LIGHT_BLUE << "You have " << queries.size() << " unanswered queries:" << RESET_COLOR << endl;

    // Display queries with serial numbers
    for (size_t i = 0; i < queries.size(); ++i) {
        cout << i + 1 << ". " << queries[i][0] << ", " << queries[i][1] << endl;
    }

    // Ask user to select a query to respond to
    int choice;
    cout << "Select the serial number of the query you want to respond to: ";
    cin >> choice;

    if (choice < 1 || choice > queries.size()) {
    ...
    // Append the response to the selected query in queries.csv
    queries[choice - 1].push_back(response);
    
    // Save updated queries back to the file
    ofstream file(filename);
    ...
    cout << LIGHT_BLUE << "Response recorded successfully." << RESET_COLOR << endl;
}
...
int main() {
    ...
    cout << "Press 7 to view a complaint." << endl;
    cout << "Press 8 to reply to a query." << endl;
    cout << "Press 9 to Exit." <<  RESET_COLOR << endl;
    cin >> choice;
